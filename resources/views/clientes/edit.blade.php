    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('clientes.update', $cliente->pk_cliente) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input name="domicilio" id="domicilio" type="text" value="{{ old('domicilio', $cliente->domicilio) }}" required
                                class="form-input rounded-md shadow-sm mt-1 block w-full" />
                        </div>

                        <div class="md:col-span-2">
                            <label class="block font-medium text-sm text-gray-700" for="domicilio_notificaciones">
                                Domicilio para notificaciones
                            </label>
                            <textarea name="domicilio_notificaciones" id="domicilio_notificaciones" rows="3"
                                class="form-textarea rounded-md shadow-sm mt-1 block w-full">{{ old('domicilio_notificaciones', $cliente->domicilio_notificaciones) }}</textarea>
                        </div>

                        <div>
                            <label class="block font-medium text-sm text-gray-700" for="fijo">Teléfono fijo</label>
                            <input name="fijo" id="fijo" type="text" value="{{ old('fijo', $cliente->fijo) }}"
                                class="form-input rounded-md shadow-sm mt-1 block w-full" />
                        </div>

                        <div>
                            <label class="block font-medium text-sm text-gray-700" for="celular">Celular</label>
                            <input name="celular" id="celular" type="text" value="{{ old('celular', $cliente->celular) }}"
                                class="form-input rounded-md shadow-sm mt-1 block w-full" />
                        </div>

                        <div>
                            <label class="block font-medium text-sm text-gray-700" for="correo">Correo</label>
                            <input name="correo" id="correo" type="email" value="{{ old('correo', $cliente->correo) }}"
                                class="form-input rounded-md shadow-sm mt-1 block w-full" />
                        </div>

                        <div>
                            <label class="block font-medium text-sm text-gray-700" for="banco">Banco</label>
                            <input name="banco" id="banco" type="text" value="{{ old('banco', $cliente->banco) }}"
                                class="form-input rounded-md shadow-sm mt-1 block w-full" />
                        </div>

                        <div>
                            <label class="block font-medium text-sm text-gray-700" for="cuenta">Cuenta</label>
                            <input name="cuenta" id="cuenta" type="text" value="{{ old('cuenta', $cliente->cuenta) }}"
                                class="form-input rounded-md shadow-sm mt-1 block w-full" />
                        </div>

                        <div>
                            <label class="block font-medium text-sm text-gray-700" for="clabe">CLABE</label>
                            <input name="clabe" id="clabe" type="text" value="{{ old('clabe', $cliente->clabe) }}"
                                class="form-input rounded-md shadow-sm mt-1 block w-full" />
                        </div>
